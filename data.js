const SURVEY={"schema":"tace-pages-v1.9-live-5docs","version":"v1.9","participant_count":50,"per_participant":5,"document_count":100,"per_document":[2,3],"assignment_version":"v1.9-public5-20260914","questions":[{"text":"Q1. 이 설명은 필요한 내용을 충분히 포함하고 있었다.","rev":false},{"text":"Q2. 이 설명은 간결하고 이해하기 쉬웠다.","rev":false},{"text":"Q3. 이 설명은 나를 배려하고 있다는 느낌을 주었다.","rev":false},{"text":"Q4. 전반적으로 이 설명에 만족한다.","rev":false}],"scale":["전혀 그렇지 않다","그렇지 않다","보통이다","그렇다","매우 그렇다"]};
const SurveyMath={
 ...
 summary(data){
  ...
  const targets={};for(const a of data.assignments)for(const doc of a.documents)targets[doc.case_code]=(targets[doc.case_code]||0)+1;
  const documents=Object.keys(targets).sort(),expectedTotal=data.assignments.reduce((n,a)=>n+a.documents.length,0);
 ...
 report(data){
  ...
  for(const [label,values] of [['응답 있는 문서',d.equalDocument],['배정 응답 완료 문서',d.completeDocument]])values.forEach((x,i)=>{
   ...
   add('문서동일가중',k,label+' 평균',x.mean,x.n,'문서별 평균에 동일 가중');
   add('문서동일가중',k,label+' 평균 간 SD',x.sd,x.n,'문서평균의 표본 SD; 표준오차/CI 아님');
   add('문서동일가중',k,label+' 긍정응답률',x.positive===null?null:x.positive*100,x.n,'문서별 4~5점 비율의 동일 가중 평균 (%)');
  });
  add('해석','문항 통계','분석 단위','','','반복 평가의 기술통계. 250건을 독립 표본으로 간주한 신뢰구간·p값은 계산하지 않음');
