...
const SurveyMath={
 ...
 summary(data){
  ...
  const seen=new Set(),people=data.participants.filter(p=>{if(p.eligible_for_statistics===false||p.consent!==true||seen.has(p.participant_id))return false;seen.add(p.participant_id);return true;});
